...
/-!
The Gaussian construction of `sepsolve.v`, using functions for vectors and lists
...
def hinge_to {n : Nat} (c : ℝ) (a : Vec n) (L : List (Gaussian n))
    (g : Gaussian n) : ℝ := (L.map (fun h => gaussian_pair_hinge c a h g)).sum
